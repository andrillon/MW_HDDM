...
%     RTs=[RTs; test_res(:,10)-test_res(:,8)];
   ...
   fprintf('... ... %g %% waves discarded because of frequency\n',mean(all_freq>max_Freq)*100)
    ...
    clean_start_trial(mindistPr<-20 | mindistPr>0)=[];
    probeidx(mindistPr<-20 | mindistPr>0)=[];
    % Pupil
    load([pupil_path filesep 'wanderIM_eyelink_S' SubID '_clean.mat'])
    
    % Loop across trials
    fprintf('%3.0f%%\n',0)
    hddm_subj=nan(length(clean_start_trial),10+length(myElecs));
    if check
        temp_ERP=[];
        temp_ERP2=[];
    end
...
%             this_endTr=this_begTr+500;
        ...
        wvidx=double(these_Waves(:,5))+start_probe(this_probeidx)-20*500;
        ...
        if check==1
            for i=1:length(find_waves)
                if these_Waves(find_waves(i),3)==24 && these_Waves(find_waves(i),5)>501 && these_Waves(find_waves(i),5)<9500
                    temp_eeg=temp_data(nE,these_Waves(find_waves(i),5)+(-1*D.fsample:1*D.fsample),this_probeidx);
                    temp_eeg=temp_eeg-mean(temp_eeg(1:D.fsample/2));
                    temp_ERP=[temp_ERP ; temp_eeg];
                end
            end
            
            this_begTr2=this_begTr-start_probe(this_probeidx)+20*500;
            if this_begTr2>101 && this_begTr2<9700
        ...
        hddm_subj(nTr,:)=[n this_blockidx this_trialidx this_probeidx this_blockcond this_cat this_perf this_code this_rt this_waveF this_pupav];
    end
    %     hddm_res=[hddm_res ; hddm_temp(~isnan(hddm_temp(:,1)),:)];
    hddm_subj=hddm_subj(~isnan(hddm_subj(:,1)),:);
    save([root_path filesep 'behav' filesep 'HDDM_WIM' SubID '_localsleep_pup_v3'],'hddm_subj')
    if check
...
for n=1:length(files)
    ...
    if exist([root_path filesep 'behav' filesep 'HDDM_WIM' SubID '_localsleep_pup_v3.mat'])==0
        continue;
    end
    fprintf('... %s\n',SubID)
    load([root_path filesep 'behav' filesep 'HDDM_WIM' SubID '_localsleep_pup_v3'])
...
tbl_headers={'SubID','BlockN','TrialN','ProbeN','Task','StimCat','Perf','RCode','RT'};
...
writetable(tbl_hddm,[root_path filesep 'hddm' filesep 'HDDM_WIM_localsleep_pup_Dec20_2.txt']);
