...
if redo==1
    for n=1:length(files)
        ...
        for nPr=1:length(start_probe)
            ...
            Probe_Waves=[Probe_Waves ; [repmat([n nPr this_blockidx this_probeN this_blockcond this_MS this_Vig this_pupav],size(sel_waves,1),1) sel_waves]];
        end
...
for n=1:length(files)
    ...
    All_Probe_Waves=[All_Probe_Waves ; Probe_Waves];
end

%%
% [repmat([n npr nE],length(abs(cell2mat(twa_results.channels(nE).maxnegpkamp))),1) abs(cell2mat(twa_results.channels(nE).maxnegpkamp))'+abs(cell2mat(twa_results.channels(nE).maxpospkamp))' ...
...
%     ]
%[repmat([n nPr this_blockidx this_probeN this_blockcond this_MS this_Vig this_pupav],size(sel_waves,1),1) sel_waves]
tbl_waves=array2table(All_Probe_Waves,'VariableNames',{'SubID','ContProbeN','BlockN','ProbeN','Task','State','Vig','PupAv',...
    'nSub','nPr','nCh','P2P','xNeg','xPos','xEnd','xPeakNeg','PeakNeg','xPeakPos','PeakPos','DownSlope','UpSlope','MaxAmpW','MinAmpW'});

tbl_waves.SubID=categorical(tbl_waves.SubID);
tbl_waves.Task=categorical(tbl_waves.Task);
tbl_waves.State=categorical(tbl_waves.State);
tbl_waves.nCh=categorical(tbl_waves.nCh);
% for nC=1:63
%     tbl_waves.nCh(tbl_waves.nCh==num2str(nC))=layout.label{nC};
% end
...
temp_topoMB=[];
for nC=1:63
    mdl=fitlme(tbl_waves(tbl_waves.nCh==layout.label{nC},:),'UpSlope~1+Task+Vig+(1|SubID)');
    temp_topoMW(nC,:)=double(mdl.Coefficients(3,2:end));
    %     temp_topoMB(nC,:)=double(mdl.Coefficients(4,2:end));
end
figure;
% subplot(1,2,1);
simpleTopoPlot_ft(temp_topoMW(:,3), path_PsychFTlayout,'on',[],0,1);
colorbar; caxis([-1 1]*5);
title('')

%     subplot(1,2,2);
% simpleTopoPlot_ft(temp_topoMB(:,3), path_PsychFTlayout,'on',[],0,1);
%     colorbar; caxis([-1 1]*5);
%     title('')
%%
...
for nState=1:3
    ...
    temp_topo=[];
    for nC=1:63
        temp_topo(nC)=nanmean(tbl_waves.P2P(tbl_waves.State==num2str(nState) & tbl_waves.nCh==layout.label{nC}));
    end
    simpleTopoPlot_ft(temp_topo', path_PsychFTlayout,'on',[],0,1);
    colorbar; caxis([0 1]*60);
    title('')
end
...
% temp=[];
% for nCh=1:63
%     temp(1,nCh)=sum(tbl_waves.nCh(tbl_waves.SubID=='332' & tbl_waves.ContProbeN==1)==num2str(nCh));
% end
% temp(2,:)=table2array(tbl_hddm(tbl_hddm.SubID=='332' & tbl_hddm.ContProbeN==1,6:68));
nout1=histc(tbl_waves.xNeg(tbl_waves.State=='1')/500,-20:1:0);
nout1=nout1./sum((tbl_waves.State=='1'));

nout2=histc(tbl_waves.xNeg(tbl_waves.State=='2')/500,-20:1:0);
nout2=nout2./sum((tbl_waves.State=='2'));

nout3=histc(tbl_waves.xNeg(tbl_waves.State=='3')/500,-20:1:0);
nout3=nout3./sum((tbl_waves.State=='3'));

figure;
bar((-20:1:0)-0.2,nout1,'BarWidth',0.2); hold on;
bar(-20:1:0,nout2,'BarWidth',0.2); hold on;
bar((-20:1:0)+0.2,nout3,'BarWidth',0.2); hold on;
